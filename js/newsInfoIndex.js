...
var queryLocalHostImage=moviesUrl+"/util/queryLocalHostImage";
var updateNews=moviesUrl+"/news/info/updateNewsInfo";
...
function  queryId(ids) {
    $.ajax({
        url:queryMenuEvaluateDetail+"?id="+ids,
        type:"POST",
        data:null,
        timeout:3000,
        success:function (data) {
            if (data.code==200){
                if (data.data!=null){
                    ...
                    localStorage.setItem("newsInfoDO",JSON.stringify(data.data));
                    window.location.href="../background/updateNews.html";
                }else {
                    alert(data.msg);
                }
            }else {
...
$(document).ready(function(){
    var data2=localStorage.getItem("newsInfoDO");
    var  datas=JSON.parse(data2);
    if(datas!=null){
        id=datas.id;
        $("#id").val(id);
        userEvaluateId=id;
        $("#menuName").val(datas.title);
        $("#images").attr("src",queryLocalHostImage+"?pathName="+datas.coverImg);
        $("#demo").val(datas.contentInfo);
